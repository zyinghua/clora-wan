#!/usr/bin/env python3
"""
Extract frames from ablation_runs videos for failed examples.
For each video file name across 6 blocks (5-1 through 5-6):
- Extract 2 frames per block with equal stride
Total: 6 blocks * 2 frames = 12 frames per video file name
"""
...
VIDEO_INFO = {
    "a_video_of_a_brown_dog_walking-a_video_of_a_grey_dog_walking.mp4": "style",
}

BLOCKS = ["5-1", "5-2", "5-3", "5-4", "5-5", "5-6"]
BASE_DIR = Path("/users/erluo/scratch/clora-wan/ablation_runs")
OUTPUT_DIR = Path("/users/erluo/scratch/clora-wan/failed_examples")

# Frame indices to extract (40, 80 for 81-frame videos with equal stride)
FRAME_INDICES = [40, 80]
...
def main():
    ...
    for video_name, category in VIDEO_INFO.items():
        ...
        # Extract 2 frames from each block
        for block in BLOCKS:
            ...
            extract_frame(video_path, FRAME_INDICES[0], frame1_path)
            
            # Extract frame at index 80
            frame2_path = video_subfolder / f"{block}_frame2.png"
            extract_frame(video_path, FRAME_INDICES[1], frame2_path)
